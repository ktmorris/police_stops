...
lapply(c(1:23), function(i){
  print(i)
  pre <- readRDS("temp/real_pre_match_hills_no_prior.rds") %>% 
  ...
  mout <- Matchby(Tr = pre$treated, X = X,
                  ...
                  exact = c(rep(T, 4), rep(F, 14)), ties = F)
  ...
  saveRDS(matches, paste0("temp/matches_rob_", i, "_no_prior.rds"))
})
...
full_inters <- rbindlist(lapply(c(1:23), function(i){
  matches <- readRDS(paste0("temp/matches_rob_", i, "_no_prior.rds"))
...
saveRDS(full_inters, "temp/rob_intervals_no_prior.rds")

full_inters <- readRDS("temp/rob_intervals_no_prior.rds")
...
p
saveRDS(p, "temp/black_overall_effect_no_prior.rds")
...
p <- ggplot(inter_data, aes(x = month, y = estimate, shape = var, linetype = var)) + geom_point() +
  ...
  geom_hline(yintercept = 0, linetype = "dashed")
p
saveRDS(p, "temp/black_relative_effect_no_prior.rds")
...
p <- ggplot(inter_data, aes(x = month, y = estimate, shape = var, linetype = var)) + geom_point() +
  ...
  geom_hline(yintercept = 0, linetype = "dashed")
p
saveRDS(p, "temp/non_black_effect_no_prior.rds")
########################################
...
saveRDS(p2, "temp/overall_effect_no_prior.rds")
